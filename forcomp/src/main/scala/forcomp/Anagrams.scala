...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(elem => elem.toLower).toList.map(x => (x._1, x._2.length)).sorted
  ...
  def combinations(occ: Occurrences): List[Occurrences] = {
    if (occ.isEmpty) List(Nil)
      ...
      for {
        no <- 0 to occ.head._2
        rest <- combinations(occ.tail)
      } yield ((occ.head._1, no) :: rest).filter(_._2 > 0)
    }.toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def substractPair(p1: (Char, Int), p2: Option[(Char, Int)]) = p2 match {
      case None => p1
      case Some((cy, ny)) => (cy, p1._2 - ny)
    }

    if (y.isEmpty) x
    else {
      for {x1 <- x} yield substractPair(x1, y.find(_._1 == x1._1))
    }.filter(_._2 > 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def fromOccurrences(occ: Occurrences): List[Sentence] = occ match {
      case List() => List(Nil)
      case _ => for {
        combination <- combinations(occ)
        word <- dictionaryByOccurrences.getOrElse(combination, Nil)
        sentence <- fromOccurrences(subtract(occ, wordOccurrences(word)))
        if combination.nonEmpty
      } yield word :: sentence
    }

    fromOccurrences(sentenceOccurrences(sentence))
  }
